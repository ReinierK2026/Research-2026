...
| # | Step | Owner | Status | Notes |
|---|---|---|---|---|
| 1.1 | Run FinBERT-ESG-9-Categories on `_E` files → ESG topic classification per passage | technical-researcher | ✅ Done | Completed 2026-06-08 (2nd run — first run data lost in DB header-corruption; fixed with `db_utils.py` v2). 680/680 2024 English rows filled. Dominant factor distribution: gov=350 (51%), soc=190 (28%), env=79 (12%), other=61 (9%). DB cols filled: `finbert_env_pct`, `finbert_soc_pct`, `finbert_gov_pct`, `finbert_other_pct`, `finbert_esg_sentences_n`, `finbert_dominant_factor`. |
| 1.2 | Run ClimateBERT on `_E` files → climate-related disclosure detection | technical-researcher | ✅ Done | Completed 2026-06-08 (2nd run — first run overwritten by concurrent FinBERT save; re-run sequentially). 680/680 filled; 678 non-zero (2 companies had 0 climate sentences, valid). Mean `climatebert_climate_pct`=0.502; 324 companies above 0.5 threshold. DB cols filled: `climatebert_climate_pct`, `climatebert_climate_sentences_n`, `climatebert_total_sentences_n`. |
...
| # | Step | Owner | Status | Notes |
|---|---|---|---|---|
| 2.1 | BGE-M3 multilingual semantic topic matcher (Chinese/bilingual) | technical-researcher | 🟡 Ready to run | Script: `scripts/phase2_nlp_local/phase2_step2_1_bge_2024.py`. Requires: `pip install FlagEmbedding sentence-transformers torch`. Model: BAAI/bge-m3 (~2.3 GB). 33 bilingual GRI topic descriptors (zh+en). New cols: bge_top1_topic, bge_top1_sim, bge_top3_topics, bge_mean_sim, bge_env_affinity, bge_soc_affinity, bge_gov_affinity. Output JSONL: bge_2024_matches.jsonl. Run sequentially (first). |
| 2.2 | XLM-RoBERTa-XNLI zero-shot ESG classifier (Chinese/bilingual) | technical-researcher | 🟡 Ready to run | Script: `scripts/phase2_nlp_local/phase2_step2_2_xlmr_2024.py`. Requires: `pip install transformers torch`. Model: MoritzLaurer/mDeBERTa-v3-base-mnli-xnli (~1.1 GB). Sentence-level zero-shot E/S/G/Other classification. New cols: xlmr_env_pct, xlmr_soc_pct, xlmr_gov_pct, xlmr_other_pct, xlmr_esg_sentences_n, xlmr_dominant_factor. Run after step 2.1. |
