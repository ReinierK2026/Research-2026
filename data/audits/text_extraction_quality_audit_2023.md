...
**Preprocessing status:** All 708 original native PDFs plus 495 expansion-batch native PDFs have been re-extracted using PyMuPDF with coordinate-aware reading-order correction. All 19 original scanned files and 13 expansion-batch scanned files were OCR'd with Tesseract. GRI content-index data has been extracted from all PDFs using the improved fitz regex pipeline. **The corpus is now fully preprocessed and ready for NLP analysis.** See the [Processing Log](#processing-log) at the end of this document.
