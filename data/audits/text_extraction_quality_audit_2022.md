...
**Corpus expansion (2026-06-10 — Entry 10):** 406 additional PDFs were sourced and processed in two batches, expanding the corpus from 623 to **1,023 files**. Batch 1 (403 PDFs): 378 native-text (PyMuPDF), 14 scanned (Tesseract OCR), 11 corrupt 0-page (hard exclusions). Batch 2 (3 additional PDFs discovered on re-check: 6 native + 2 scanned, 1 corrupt — `2013_2022`). The expansion added predominantly Chinese-only reports — reducing the English `_E` share from 62.4% to 39.0%. Quality verification yielded a 2.8% flag rate, with 2 genuine near-empty hard exclusions (3413_2022: 196 chars; 2832_2022: 281 chars). GRI extraction now covers **992 non-OCR PDFs**: 878/992 (88.5%) with codes, confirming pipeline consistency with the original corpus (87.9%). No further deduplication was applied; apply `_E`-preferred dedup before modelling.
...
### Phase 2 — NLP Pipeline: Multilingual Track (Chinese/bilingual files, 624 total: 225 original ✅ + ~399 expansion ⏳)

| # | Step | Status | Notes |
|---|---|---|---|
| 2.1 | BGE-M3 multilingual semantic topic matcher | ✅ Done 2026-06-09 (original) | **Original batch: 225/225 filled.** Top1: GRI Alignment(70), Stakeholder Engagement(31), Training & Education(22). Mean sim=0.654. Affinity: gov=0.308, soc=0.299, env=0.126. Script: `phase2_step2_1_bge_2022.py`. ⚠️ Expansion batch (~392 files) ⏳ Pending. |
| 2.2 | XLM-RoBERTa-XNLI zero-shot ESG classifier | ✅ Done 2026-06-09 (original) | **Original batch: 225/225 filled.** Dominant: soc=175 (77.8%), other=17 (7.6%), env=20 (8.9%), gov=13 (5.8%). Mean 47.2 sentences. Script: `phase2_step2_2_xlmr_2022.py`. ⚠️ Expansion batch (~392 files) ⏳ Pending. |
| 2.3 | Block C indicators (Chinese/bilingual) | ✅ Done 2026-06-09 (original) | **Original batch: 224 files processed.** Combined corpus counts based on original batch only. mat_found=581, board_approved=349, double_mat=47, ai_tool=5. Script: `phase2_block_c_chinese_2022.py`. ⚠️ Expansion batch (~392 files) ⏳ Pending. |

> **⚠️ Corpus expansion (Entry 10, 2026-06-10) — Phase 2 NLP pending for ~392 new CN files:**  
> Total CN files expanded from 225 → 624. EXCLUDE set: `{1795, 3704, 9917, 2832, 3413, 3014, 3016}` (7 tickers: corrupt/near-empty). Processable expansion files: **~392**.  
> - **2.1 BGE-M3:** ⏳ Pending local GPU — re-run `phase2_step2_1_bge_2022.py` (resumes from progress JSON; expansion tickers not in `done` set will be picked up automatically)  
> - **2.2 XLMR:** ⏳ Pending local GPU — re-run `phase2_step2_2_xlmr_2022.py` (same resume logic)  
> - **2.3 Block C Chinese:** ⏳ Pending local GPU — re-run `phase2_block_c_chinese_2022.py`  
> After completion: re-run `phase3_2022.py` to update `topic_depth_score` for expansion tickers. All scripts updated with EXCLUDE set above.
...
| 3.2 | Populate `n_material_topics_b` from GRI codes summary | data-analyst | ✅ Done 2026-06-09 | **586/632 (92.7%)** — current DB (632 rows, post-restructure). Sourced from `gri_codes_summary_2022.csv` (no `gri_tables_2022/` dir). Script: `phase3_2022.py`. *(Previous figure 517/980 was pre-DB-restructure denominator.)* |
| 3.3 | Compute `mda_index` per Padilla-Garrido et al. (2024) | data-analyst | ✅ Done 2026-06-09 | **606/632 (95.9%)** — current DB. Mean=0.601, mode=0.6. 10-item binary index from Block C columns. Script: `phase3_2022.py`. *(Previous figure 608/980 was pre-DB-restructure denominator.)* |
| 3.4 | Compute `topic_depth_score` from NLP semantic similarity | data-analyst | ✅ Done 2026-06-09 | **611/632 (96.7%)** — current DB. Mean≈0.590. English ESGLens mean≈0.231; Chinese BGE mean≈0.643 (model calibration gap). Will increase after CN expansion NLP (BGE) runs. Script: `phase3_2022.py`. |
| 3.5 | Compute `gri_content_index_completeness` from GRI codes | data-analyst | ✅ Done 2026-06-09 | **514/632 (81.3%)** — current DB. Mean=0.826, median=0.882. Denom=34 (Universal 2021). Script: `phase3_2022.py`. |
