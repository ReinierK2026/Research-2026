...
| 2.3 | Block C indicators (Chinese/bilingual) — original 172 CN files | ✅ Done 2026-06-09 | 172 files processed. Script: `phase2_block_c_chinese_2021.py`. |
| 2.3s | Block C supplement — 14 additional supplement-track tickers | ✅ Done 2026-06-10 (sandbox) | Tickers: 2329 2441 2449 6770 1536 2012 2023 2102 2206 2345 2707 6189 6768 8112. All 491 DB rows now have Block C nonblank. mat_section_found=1: 462/491 (94.1%); mat=0: 29 (report lacking materiality section). Also set report_language='zh' for 11 blank-language supplement tickers. Script: `block_c_gap10_fix.py` (inline sandbox). |
...
| 3.2 | Populate `n_material_topics_b` from GRI codes summary | data-analyst | ✅ Done 2026-06-09 | **est. ~319/491** (pre-restructure numerator over updated denom; DB restructured 822→491; phase3_2021.py pending re-run on current DB for exact counts). Mean=15.9, median=16. No `gri_tables_2021/` directory — sourced from `gri_codes_summary_2021.csv`. Script: `phase3_2021.py`. |
| 3.3 | Compute `mda_index` per Padilla-Garrido et al. (2024) | data-analyst | ✅ Done 2026-06-09 | **est. ~491/491** (Block C 491/491 nonblank; DB restructured 822→491 — phase3_2021.py pending re-run to confirm). Mean=0.549, mode=0.5. Pre-CSRD era; `double_materiality_mentioned` near-zero (0.7%) as expected. Script: `phase3_2021.py`. |
| 3.4 | Compute `topic_depth_score` from NLP semantic similarity | data-analyst | ✅ Done 2026-06-09 | **est. ~486/491** (BGE coverage 486/491; DB restructured 822→491 — phase3_2021.py pending re-run to confirm). Mean=0.577, median=0.586. English ESGLens mean≈0.231; Chinese BGE mean≈0.643 (model calibration gap). Script: `phase3_2021.py`. |
| 3.5 | Compute `gri_content_index_completeness` from GRI codes | data-analyst | ✅ Done 2026-06-09 | **41/491** (early adopters preserved; DB restructured 822→491 — phase3_2021.py pending re-run to confirm). Mean=0.237 (Universal 2021 reporters only). **Expected**: ~450/491 rows are GRI Standards 2016 reporters → GCI=0.0 by design. Filter by `gri_standard_version` when analysing cross-cohort. Script: `phase3_2021.py`. |
...
*Last updated: 2026-06-10 (Entry 15 — Block C supplement 14 tickers; report_language fix 11 tickers; XLMR 13-ticker gap reclassified as processing error with local re-run instructions; 6770 Phase 1 NLP gap documented; full 2021 coverage table updated)*
