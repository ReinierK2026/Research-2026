...
| 8 | **mpqi_p2–o1 collinearity** (r=0.809) — both items capture visualization quality; high correlation is expected given shared construct | Medium | ✅ Explained (2026-06-23) — collinearity is by design: p2 and o1 both measure aspects of materiality visualization. Decision: o1 excluded from the final 9-item MPQI and consolidated into p2 (renamed "visualization quality (consolidated)"). See `academic-researcher_mpqi-full-results_2026-06-23.md` Step 1. No cross-dimension collinearity remains in the 9-item index (all remaining pairs r < 0.30 except s2–s3 within same dimension). Report in measurement section as a deliberate consolidation decision, not an unresolved limitation. |
...
21. ✅ **Stream B IPW check (2026-06-24)** — IPW-weighted CS21 ATT ≈ unweighted ATT. DR estimator with outcome regression already absorbs the size imbalance. AUC=0.775 reported as a limitation; overlap assumption stressed but DR robustness confirmed. Issue #3 closed.
