...
|---|---|---|---|---|---|
| **H1** | `n_material_topics_b` | **Negative ATT** | −2 to −5 topics (mean pre-adoption ≈ 15.0); final R run: ATT=−8.62* (SE=3.907) | CS21 `att_gt()` | Confirmatory — pending OSF |
...
| **R3** | **Rambachan-Roth HonestDiD** (`HonestDiD::createSensitivityResults()`) | **NON-NEGOTIABLE** — with only 44 controls at primary ATT cell, post-treatment parallel trends cannot be empirically tested; bounds on how large a violation must be to overturn the result | ✅ Complete (R CSVs, 2026-06-24) | **H1 robust at M=1.75 (ub=−0.174).** At M=2.0 the upper bound crosses zero (ub=+0.162, not significant). Paper statement: "Robust to violations up to M=1.75 times the largest pre-trend difference." |
